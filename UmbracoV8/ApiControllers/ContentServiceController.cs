...
namespace UmbracoV7.ApiControllers
{
    public class ContentServiceController : UmbracoApiController
    {
        ...
        private List<int> CreateTestNodesUnderParent(int quantity, int parentId)
        {
            ...
            {
                var newItem = Services.ContentService.Create(Guid.NewGuid().ToString(), parentId, TestNodeAlias);
                var publishAttempt = Services.ContentService.SaveAndPublish(newItem);
        ...
        {
            Services.ContentService.DeleteOfType(Services.ContentTypeService.Get(TestNodeAlias).Id);
